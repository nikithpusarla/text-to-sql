...
async function ask(query) {
  ...
  if (!response.ok) throw new Error(await errorMessage(response, 'The query could not be processed.'));
...
function renderQuestions(questions) {
  const cards = questions.map(question => `<div class="question-card"><p>CLARIFYING ${escapeHtml(question.slot_name.toUpperCase())}</p><h3>${escapeHtml(question.question_text)}</h3><div class="options">${question.options.map(option => `<button class="option" data-slot="${escapeHtml(question.slot_name)}" data-value="${escapeHtml(option)}">${escapeHtml(option.replaceAll('_', ' '))}</button>`).join('')}</div></div>`).join('');
...
async function clarify(slotName, value) {
  ...
  if (!response.ok) throw new Error(await errorMessage(response, 'The clarification could not be processed.'));
  const data = await response.json();
  if (data.questions?.length) return renderQuestions(data.questions);
  return renderResult(data.result);
}
...
async function renderResult(result) {
  ...
  if (!response.ok) throw new Error(await errorMessage(response, 'The answer could not be loaded from the database.'));
  const data = await response.json();
  ...
  const rows = data.rows.slice(0, 20).map(row => `<tr>${columns.map(column => `<td>${escapeHtml(row[column] ?? '')}</td>`).join('')}</tr>`).join('');
  showConversation(`<div class="answer-card"><div class="result-label"><strong>Your answer</strong><span>${escapeHtml(data.row_count)} ROWS</span></div>${data.rows.length ? `<table class="answer-table"><thead><tr>${header}</tr></thead><tbody>${rows}</tbody></table>` : '<p class="loading">No matching rows found.</p>'}<details><summary>View generated SQL</summary><pre class="sql">${escapeHtml(data.sql)}</pre></details></div>`);
}
